...
def treinar_nbeats(X_treino, y_treino, X_val, y_val, num_exec, execucao, idade):
    ...
    for rodada in range(num_exec):
        ...
        for neuronio in neuronios:
            ...
            for iteracao in iteracoes:
                clear_session()
                model = build_model(qtd_lags_sel, 10, neuronio)
                ...
                if novo_mse < melhor_mse:   
                    melhor_mse = novo_mse
                    melhor_modelo = model
                    print('Execução:',execucao,'idade:', idade,'rodada:', rodada,'melhor configuração neuronios:', neuronio, 'RMSE:', melhor_mse)
                    model.save(f'Resultados - n-beats - Artigo - MIMO/model_n_beats_rodada_{rodada}_idade_{idade}')
                    del model
                    clear_session()
                    gc.collect()
    ...
    resultados_df.to_csv(f'Resultados - n-beats - Artigo - MIMO/n_beats_idade_{idade}_resultados_df.csv')
    # obter o melhor modelo
    melhor_mse_i = resultados_df['rmse'].argmin()
    melhor_rodada = int(resultados_df.iloc[melhor_mse_i]['rodada'])
    melhor_modelo = tf.keras.models.load_model(f'Resultados - n-beats - Artigo - MIMO/model_n_beats_rodada_{melhor_rodada}_idade_{idade}')
...
def main(qtd_execucoes=1):
    ...
    for execucao in range(1, qtd_execucoes+1):
        ...
        for idade, i in zip(idades, range(0,len(idades)+1)):
            ...
            treino_i_norm = lnmx_norm[:-10] # removendo as 10 ultimas obs 
            # Separando os dados em X, y e utilizando 2 lags 

            X, y = split_sequence(treino_i_norm, 2, 10)
            X_train, y_train, X_val, y_val = divisao_dados_temporais(X, y, perc_treino = 0.80)
            
            # treinamento
            n_beats_model = treinar_nbeats(X_train, y_train, X_val, y_val, 10, execucao, idade)
            
            # previsao
            X_test_pred = treino_i_norm[-2:].reshape(1, 2) # 2 lags
            predictions = prev_n_beats(n_beats_model, X_test_pred).reshape(10,)
            ...
            print('-----------------------------------------------')
            print(f'IDADE: {idade}')
            print('RMSE para o Modelo n-beats MIMO = ', rmse_n_beats[0, i])
            print('-----------------------------------------------')
        ...
        previsoes_n_beats.to_csv(f'Resultados - n-beats - Artigo - MIMO/previsoes_n_beats_{execucao}.csv', index=None, header=True, encoding = 'utf-8')
        rmse_n_beats.to_csv(f'Resultados - n-beats - Artigo - MIMO/rmse_n_beats_{execucao}.csv', index=None, header=True, encoding = 'utf-8')
        
if __name__ == '__main__':
    print('Iniciando o script N-Beats mimo')
    main()
    print('FIM!')
