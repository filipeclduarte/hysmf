...
def treinar_nbeats(X_treino, y_treino, X_val, y_val, num_exec, execucao, idade):
        ...
        for neuronio in neuronios:
            ...
            for iteracao in iteracoes:
                ...
                if novo_mse < melhor_mse:   
                    ...
                    model.save(f'Resultados - hibrido - n-beats - Artigo - MIMO/model_n_beats_rodada_{rodada}_idade_{idade}')
                    del model
                    clear_session()
                    gc.collect()
    ...
    resultados_df.to_csv(f'Resultados - hibrido - n-beats - Artigo - MIMO/n_beats_idade_{idade}_resultados_df.csv')
    # obter o melhor modelo
    melhor_mse_i = resultados_df['rmse'].argmin()
    melhor_rodada = int(resultados_df.iloc[melhor_mse_i]['rodada'])
    melhor_modelo = tf.keras.models.load_model(f'Resultados - hibrido - n-beats - Artigo - MIMO/model_n_beats_rodada_{melhor_rodada}_idade_{idade}')
...
def main(qtd_execucoes=1):
    ...
    for execucao in range(1, qtd_execucoes+1):
        ...
        for idade, i in zip(idades, range(0,len(idades)+1)):
            ...
            n_beats_model_error = treinar_nbeats(X_train_error, y_train_error, X_val_error, y_val_error, 10, execucao, idade)
              
            # Realizar a previsão
            X_test_pred = arima_error_norm[-2:].reshape(1, 2)
            n_beats_predict_error = prev_n_beats(n_beats_model_error, X_test_pred).reshape(10, )
            ...
            n_beats_predict_error = desnormalizar(n_beats_predict_error, arima_error_train_test)
            ...
            print('-----------------------------------------------')
            print(f'IDADE: {idade}')
            print('RMSE para o Modelo n-beats MIMO = ', rmse_hibrido[0, i])
            print('-----------------------------------------------')
        ...
        previsoes_hibrido.to_csv(f'Resultados - hibrido - n-beats - Artigo - MIMO/previsoes_hibrido_{execucao}.csv', index=None, header=True, encoding = 'utf-8')
        rmse_hibrido.to_csv(f'Resultados - hibrido - n-beats - Artigo - MIMO/rmse_hibrido_{execucao}.csv', index=None, header=True, encoding = 'utf-8')
        
if __name__ == '__main__':
    print('Iniciando o script Hibrido N-Beats mimo')
    main()
    print('FIM!')
